...
const STRIPE_PRO_LINK = process.env.NEXT_PUBLIC_STRIPE_PRO_LINK ?? "#pricing";
const STRIPE_BIZ_LINK = process.env.NEXT_PUBLIC_STRIPE_BUSINESS_LINK ?? "#pricing";

/* ── Paywall Modal ── */
function PaywallModal({ onClose, onActivate }: { onClose: () => void; onActivate: () => void }) {
  ...
  const submitCode = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setCodeError("الكود غير صحيح. تحقق منه وحاول مجدداً.");
      }
    } catch {
      setCodeError("حدث خطأ. حاول مجدداً.");
    } finally {
      setChecking(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
      ...
      <div className="float-in w-full max-w-2xl">
        <div className="glass rounded-3xl p-6 sm:p-8" style={{ border: "1px solid rgba(139,92,246,0.35)" }}>

          {/* Header */}
          <div className="text-center mb-8">
            <div className="text-5xl mb-3">🚀</div>
            <h2 className="text-white font-black text-2xl mb-2">انتهت توليداتك المجانية!</h2>
            <p className="text-white/40 text-sm">لقد استخدمت {FREE_LIMIT} توليدات مجانية — رائع! اشترك للاستمرار</p>
          </div>
            ...
            {[
              {
                ...
                price: "$19",
                color: "#7c3aed",
                features: ["توليدات غير محدودة", "5 منصات", "4 لهجات", "دعم بريدي"],
              ...
              },
              {
                ...
                price: "$49",
                color: "#f59e0b",
                features: ["كل مزايا برو", "5 حسابات", "API access", "دعم واتساب"],
                href: STRIPE_BIZ_LINK,
                glow: false,
              },
            ].map(plan => (
              <div key={plan.name} className="rounded-2xl p-5"
                style={{
                  background: plan.glow ? "rgba(124,58,237,0.15)" : "rgba(255,255,255,0.04)",
                  border: `1px solid ${plan.glow ? "rgba(139,92,246,0.5)" : "rgba(255,255,255,0.08)"}`,
                  boxShadow: plan.glow ? "0 0 30px rgba(124,58,237,0.2)" : "none",
                }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white font-black">{plan.name}</span>
                  <span className="font-black text-xl" style={{ color: plan.color }}>{plan.price}<span className="text-xs text-white/40">/شهر</span></span>
                </div>
                ...
                <a href={plan.href} target="_blank" rel="noopener noreferrer"
                  className="block w-full py-2.5 rounded-xl text-center text-sm font-black text-white transition-all"
                  style={plan.glow ? {
          ...
          <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <p className="text-white/50 text-xs mb-3">بعد الدفع ستحصل على كود تفعيل — أدخله هنا:</p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-transparent text-white text-sm rounded-xl px-3 py-2.5 outline-none"
                style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)" }}
              />
              <button onClick={submitCode} disabled={checking || !code.trim()}
                className="px-4 py-2.5 rounded-xl text-sm font-bold text-white transition-all disabled:opacity-40"
                style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)" }}>
                {codeSuccess ? "✓" : checking ? "..." : "تفعيل"}
              </button>
            </div>
            {codeError && <p className="text-red-400 text-xs mt-2">{codeError}</p>}
            {codeSuccess && <p className="text-green-400 text-xs mt-2">✓ تم التفعيل! جاري الفتح...</p>}
          </div>

          <button onClick={onClose}
            className="w-full mt-4 py-2 text-white/30 text-xs hover:text-white/50 transition-colors">
            إغلاق — العودة للصفحة الرئيسية
          </button>
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fu = parseInt(localStorage.getItem("freeUsed") ?? "0", 10);
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!isPro) {
        ...
        setFreeUsed(newFree);
        localStorage.setItem("freeUsed", String(newFree));
        if (newFree >= FREE_LIMIT) setTimeout(() => setShowPaywall(true), 800);
      }
          ...
          <div className="flex items-center gap-2">
            ...
            {!isPro ? (
              <button onClick={() => setShowPaywall(true)}
                ...
                style={{
                  background: freeUsed >= FREE_LIMIT - 2 ? "rgba(239,68,68,0.15)" : "rgba(255,255,255,0.04)",
                  border: freeUsed >= FREE_LIMIT - 2 ? "1px solid rgba(239,68,68,0.4)" : "1px solid rgba(255,255,255,0.1)",
                  color: freeUsed >= FREE_LIMIT - 2 ? "#f87171" : "rgba(255,255,255,0.4)",
                }}>
                {freeUsed >= FREE_LIMIT ? "🔒 انتهت المجانية" : `${FREE_LIMIT - freeUsed}/${FREE_LIMIT} متبقي`}
              </button>
            ) : (
